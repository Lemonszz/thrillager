...
import party.lemons.thrillager.ThrillagerEntity.ArmPose;
...
public class ThrillagerModel extends EntityModel<ThrillagerEntity> implements IHasArm, IHasHead
{
	...
	public ThrillagerModel(float scaleFactor, float offset, int textureWidthIn, int textureHeightIn)
	{
		this.head = (new RendererModel(this)).setTextureSize(textureWidthIn, textureHeightIn);
		this.head.setRotationPoint(0.0F, 0.0F + offset, 0.0F);
		...
		this.nose.setRotationPoint(0.0F, offset - 2.0F, 0.0F);
		...
		this.body.setRotationPoint(0.0F, 0.0F + offset, 0.0F);
		...
		this.arms.setRotationPoint(0.0F, 0.0F + offset + 2.0F, 0.0F);
		...
		this.field_217143_g.setRotationPoint(-2.0F, 12.0F + offset, 0.0F);
		...
		this.field_217144_h.setRotationPoint(2.0F, 12.0F + offset, 0.0F);
		...
		this.rightArm.setRotationPoint(-5.0F, 2.0F + offset, 0.0F);
		...
		this.leftArm.setRotationPoint(5.0F, 2.0F + offset, 0.0F);
	}

	public void render(ThrillagerEntity entityIn, float limbSwing, float limbSwingAmount, float ageInTicks, float netHeadYaw, float headPitch, float scale) {
		...
		if (entityIn.getArmPose() == ArmPose.CROSSED) {
	...
	public void setRotationAngles(ThrillagerEntity entityIn, float limbSwing, float limbSwingAmount, float ageInTicks, float netHeadYaw, float headPitch, float scaleFactor) {
		...
		ArmPose pose = entityIn.getArmPose();
		...
		} else if (pose == ArmPose.SPELLCASTING) {
		...
		} else if (pose == ArmPose.CELEBRATING) {
			this.rightArm.rotationPointZ = 0.0F;
			this.rightArm.rotationPointX = -5.0F;
	...
	public void setLivingAnimations(ThrillagerEntity entityIn, float limbSwing, float limbSwingAmount, float partialTick) {
